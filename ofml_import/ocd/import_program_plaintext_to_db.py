...
class InsertOcd:
    ...
    @memoize_insert("ocd_rounding")
    def extract_rounding(self, rounding_id: str) -> OcdRoundingDB:
        df = self.ocd_rounding.df
        result = df[df["id"] == rounding_id]

        if result.empty:
            logger.warning(f"{rounding_id} yield no rounding")
            return None
        ...
        self.set_extracted(df, result)
        return OcdRoundingDB(
            nr=row.nr,
            min=row.min,
            max=row.max,
            type=row.type,
            precision=row.precision,
            add_before=row.add_before,
            add_after=row.add_after,
        )

    @memoize_insert("ocd_codescheme")
    def extract_codescheme(self, scheme_id: str) -> OcdCodeSchemeDB:
        df = self.ocd_codescheme.df
        result = df[df["scheme_id"] == scheme_id]

        if result.empty:
            logger.warning(f"{scheme_id} yield no codscheme")
            return None
        ...
        self.set_extracted(df, result)
        return OcdCodeSchemeDB(
            scheme=row.scheme,
            varcode_sep=row.varcode_sep,
            value_sep=row.value_sep,
            visibility=row.visibility,
            invisible_char=row.invisible_char,
            unselect_char=row.unselect_char,
            trim=row.trim,
            mo_sep=row.mo_sep,
            mo_bracket=row.mo_bracket,
        )
    ...
    def extract_price(self, article_nr: str) -> list[OcdPriceDB]:
        ...
        return result.apply(
            lambda row: OcdPriceDB(
                ...
                ref_rounding=self.extract_rounding(row.rounding_id),
            ),
            axis=1,
        ).to_list()
    ...
    def __init__(
        ...
        self.ocd_rounding = ocd.table("ocd_rounding.csv")
        self.ocd_codescheme = ocd.table("ocd_codescheme.csv")

        for table in [self.ocd_artlongtext, self.ocd_usermessage]:
            table.df = format_text_table(table.df)

        for table in [v for k, v in self.__dict__.items() if k.startswith("ocd_")]:
            table.df["db_table_id"] = None
        ...
        for row in self.ocd_article.df.itertuples():
            ...
            article = OcdArticleDB(
                ...
                ref_codescheme=self.extract_codescheme(row.scheme_id),
                ref_program=self.program,
            )
...
if __name__ == "__main__":
    ...
    repo.read_profiles()
    p = repo.load_program("workplace")
    ocd = p.load_ofml_part("ocd")
    ocd.read_all_tables()

    with generate_session_ctx() as session:
        InsertOcd(ocd, session, p.name)
        p = session.scalars(select(OcdProgramDB)).first()
        print(p)
        input("..")
        print(
            "OcdTextDB left:",
            len(session.scalars(select(OcdTextDB)).all()),
        )

        session.add(OcdGlobalPackagingDB(program_id=1, weight_unit="xx"))
        session.commit()
        print(
            "OcdGlobalPackagingDB left:",
            len(session.scalars(select(OcdGlobalPackagingDB)).all()),
        )

        for a in session.scalars(select(OcdProgramDB)).all():
            print("DELETE program", a)
            session.delete(a)
            session.commit()

        # for a in session.scalars(
        #     select(OcdArticleDB).where(OcdArticleDB.program_id == 1)
        # ).all():
        #     print(a)
        #     session.delete(a)
        #     session.commit()
        ...
        # print(
        #     "OcdPropertyClassDB left:",
        #     len(session.scalars(select(OcdPropertyClassDB)).all()),
        # )

        # print(
        #     "OcdPropertyDB left:",
        #     len(session.scalars(select(OcdPropertyDB)).all()),
        # )

        # print(
        #     "OcdPropertyValueDB left:",
        #     len(session.scalars(select(OcdPropertyValueDB)).all()),
        # )
        ...
        # # session.execute(delete(OcdTextDB).where(OcdTextDB.text_type == TextType.PROP))
        # # session.commit()
        # # for text in session.scalars(
        # #     select(OcdTextDB).where(OcdTextDB.program_id == 1)
        # # ).all():
        # #     print(text)
